...
%**************************************************************************
% Parameters of RATSMC
%**************************************************************************
DoF = 3;                                % Degrees of freedom of the foot translation
Kp  = 15 * eye(DoF);                    % Proportional gain
Ki1 = 100 * eye(DoF);                   % Integral gain 1
Ki2 = 0.01 * eye(DoF);                  % Integral gain 2
p = 9;          q = 7;
alph = 0.1;
eta11 = 10;     eta12 = 10;
eta21 = 10;     eta22 = 10;
...
%**************************************************************************
% Variables corresponding to RATSMC
%**************************************************************************
...
KsiHat1 = cell(6,1);                    % Estimation of adaptive parameter1
KsiHat2 = cell(6,1);                    % Estimation of adaptive parameter2
SigmaHat = cell(6,1);                   % Estimation of the upper bound of disturbance
U_RATSMC = cell(6,1);                   % Limb-level controller
...
for legL = 1 : 6
    ...
    %--------------------------------------------------------------------------
    % Initial values corresponding to RATSMC
    %--------------------------------------------------------------------------
...
% Hybrid Control
Disturbance = trial*0.005*[cos(TimeSeq); sin(2*TimeSeq); cos(2*TimeSeq)]; % Lumped disturbance
for k = 2 : Nrhyth
    ...
    for legL = 1 : 6
        ...
        % RATSMC
        %--------------------------------------------------------------------------
        u_eq = Kp * DPd{legL}(:,k-1) + Ki1 * e{legL}(:,k-1) + Ki2 * norm(e{legL}(:,k-1),1)^(q/p) * sign(e{legL}(:,k-1)); % Equivalent control part
        u_sw = (KsiHat1{legL}(1,k-1) + SigmaHat{legL}(1,k-1)) * norm(s{legL}(:,k-1),1)^(alph) * sign(s{legL}(:,k-1)) + (KsiHat2{legL}(1,k-1) + SigmaHat{legL}(1,k-1)) * s{legL}(:,k-1) + SigmaHat{legL}(1,k-1) * Kp * sign(s{legL}(:,k-1)); % Switching control part
        U_RATSMC{legL}(:,k) = Kp \ (u_eq + u_sw);
        invJacobian = FUNCSET.INVJACOBI(Measure,alphaL,q_const{legL}(:,k-1));           % Inverse Jacobian
        Dq_ideal{legL}(:,k) = invJacobian * ( U_RATSMC{legL}(:,k) + Disturbance(:,k) ); % Plant
        ...
        e_integ{legL}(:,k) = e_integ{legL}(:,k-1) + (Ki1 * e{legL}(:,k) + Ki2 * norm(e{legL}(:,k),1)^(q/p) * sign(e{legL}(:,k))) * ts;
        s{legL}(:,k) = Kp * e{legL}(:,k) + e_integ{legL}(:,k);
        Ds{legL}(:,k) = (s{legL}(:,k) - s{legL}(:,k-1)) ./ ts;
        DKsiHat1 = eta11 * (norm(s{legL}(:,k),1)^(alph+1) - eta12 * KsiHat1{legL}(1,k-1));
        DKsiHat2 = eta21 * (norm(s{legL}(:,k),2)^2 - eta22 * KsiHat2{legL}(1,k-1));
        DSigmaHat = lamb1 * (norm(s{legL}(:,k),1)^(alph+1) + norm(s{legL}(:,k),2)^2 + norm(Kp*s{legL}(:,k),1) - lamb2 * SigmaHat{legL}(1,k-1));
        KsiHat1{legL}(1,k) = KsiHat1{legL}(1,k-1) + DKsiHat1 * ts;
        KsiHat2{legL}(1,k) = KsiHat2{legL}(1,k-1) + DKsiHat2 * ts;
        SigmaHat{legL}(1,k) = SigmaHat{legL}(1,k-1) + DSigmaHat * ts;
...
%\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\
% Save data
save Comparison_Data\Z_2W_trial_MPC_RATSMC                Z_2W_trial
save Comparison_Data\ZTilde_trial_MPC_RATSMC              ZTilde_trial
save Comparison_Data\Pd_trial_MPC_RATSMC                  Pd_trial
save Comparison_Data\Pa_trial_MPC_RATSMC                  Pa_trial
save Comparison_Data\P_2W_trial_MPC_RATSMC                P_2W_trial
save Comparison_Data\q_a_trial_MPC_RATSMC                 q_a_trial

...
%**************************************************************************
% Variables corresponding to PID
%**************************************************************************
...
U_PID = cell(6,1);                      % Limb-level controller
...
for legL = 1 : 6
    ...
    %--------------------------------------------------------------------------
    % Initial values corresponding to PID
    %--------------------------------------------------------------------------
...
for k = 2 : Nrhyth
    ...
    for legL = 1 : 6
        ...
        % PID
        %--------------------------------------------------------------------------
        U_PID{legL}(:,k) = DPd{legL}(:,k-1) + Kp * e{legL}(:,k-1) + Ki * e_integ{legL}(:,k-1) + Kd * e_deriv{legL}(:,k-1);
        invJacobian = FUNCSET.INVJACOBI(Measure,alphaL,q_const{legL}(:,k-1));           % Inverse Jacobian
        Dq_ideal{legL}(:,k) = invJacobian * ( U_PID{legL}(:,k) + Disturbance(:,k) );    % Plant
...
%\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\
% Save data
save Comparison_Data\Z_2W_trial_MPC_PID                Z_2W_trial
save Comparison_Data\ZTilde_trial_MPC_PID              ZTilde_trial
save Comparison_Data\Pd_trial_MPC_PID                  Pd_trial
save Comparison_Data\Pa_trial_MPC_PID                  Pa_trial
save Comparison_Data\P_2W_trial_MPC_PID                P_2W_trial
save Comparison_Data\q_a_trial_MPC_PID                 q_a_trial

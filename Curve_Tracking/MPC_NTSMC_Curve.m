...
%**************************************************************************
% Variables corresponding to NTSMC
%**************************************************************************
...
U_NTSMC = cell(6,1);                   % Limb-level controller
...
for legL = 1 : 6
    ...
    %--------------------------------------------------------------------------
    % Initial values corresponding to NTSMC
    %--------------------------------------------------------------------------
    ...
    s{legL}(:,1) = e{legL}(:,1) + diag( 1/al * abs(e{legL}(:,1)).^g1h ) * sign(e{legL}(:,1));
...
for k = 2 : Nrhyth
    ...
    for legL = 1 : 6
        ...
        %--------------------------------------------------------------------------
        for i = 1 : DoF
            U_NTSMC{legL}(i,k) = Pd{legL}(i,k-1) + (1+abs(e_deriv{legL}(i,k-1)).^(p1q-1)) / (1+g1h/al*abs(e{legL}(i,k-1)).^(g1h-1))...
                * (k1*s{legL}(i,k-1) + k2*abs(s{legL}(i,k-1)).^m1n*sign(s{legL}(i,k-1)));
        end
        invJacobian = FUNCSET.INVJACOBI(Measure,alphaL,q_const{legL}(:,k-1));           % Inverse Jacobian
        Dq_ideal{legL}(:,k) = invJacobian * ( U_NTSMC{legL}(:,k) + Disturbance(:,k) );  % Plant
        ...
        s{legL}(:,k) = e{legL}(:,k) + diag( 1/al * abs(e{legL}(:,k)).^g1h ) * sign(e{legL}(:,k));
...
%\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\
% Save data
save Comparison_Data\Z_2W_trial_MPC_NTSMC                Z_2W_trial
save Comparison_Data\ZTilde_trial_MPC_NTSMC              ZTilde_trial
save Comparison_Data\Pd_trial_MPC_NTSMC                  Pd_trial
save Comparison_Data\Pa_trial_MPC_NTSMC                  Pa_trial
save Comparison_Data\P_2W_trial_MPC_NTSMC                P_2W_trial
save Comparison_Data\q_a_trial_MPC_NTSMC                 q_a_trial
